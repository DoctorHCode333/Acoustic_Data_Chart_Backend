  ...
  try {
    ...
    let fetchTotalInteractionsQuery = `
      ...
      WHERE 
        ...
        AND (CALLDURATION >= (:callFrom) AND CALLDURATION <= (:callTo)  OR CALLDURATION is null)
    ...
    let fetchDataQuery = `SELECT COUNT(CONVERSATION_ID)as interactions,
        SUM(CASE WHEN  (CALLDURATION >= (:callFrom) AND CALLDURATION <= (:callTo)) THEN 1 ELSE 0 END) as callCount,
    ...
    const placeholdersForDivision = division
      ...
      .join(", ");
    const placeholdersForQueue = queue
      .map((_, i) => `:queue${i + 1}`)
      .join(", ");
    ...
    const placeholdersForWorkTeams = workTeams
      ...
      .join(", ");
    const placeholdersForAgentId = agentId
      .map((_, i) => `:agentId${i + 1}`)
      .join(", ");
    const placeholdersForANI = ANI.map((_, i) => `:ANI${i + 1}`).join(", ");
    const placeholdersForPartyId = partyId
      .map((_, i) => `:partyId${i + 1}`)
      .join(", ");
    ...
    if (queue.length > 0) {
      fetchTotalInteractionsQuery += ` AND TRIM(QUEUE) IN (${placeholdersForQueue})`;
      fetchDailyTotalInteractionsQuery += ` AND TRIM(QUEUE) IN (${placeholdersForQueue})`;
      fetchDataQuery += ` AND TRIM(QUEUE) IN (${placeholdersForQueue})`;
      fetchDailyDataQuery += ` AND TRIM(QUEUE) IN (${placeholdersForQueue})`;
    }
    ...
    if (workTeams.length > 0) {
      fetchTotalInteractionsQuery += ` AND TRIM(WORK_TEAMS) IN (${placeholdersForWorkTeams})`;
      fetchDailyTotalInteractionsQuery += ` AND TRIM(WORK_TEAMS) IN (${placeholdersForWorkTeams})`;
      fetchDataQuery += ` AND TRIM(WORK_TEAMS) IN (${placeholdersForWorkTeams})`;
      fetchDailyDataQuery += ` AND TRIM(WORK_TEAMS) IN (${placeholdersForWorkTeams})`;
    }

    if (agentId.length > 0) {
      fetchTotalInteractionsQuery += ` AND TRIM(AGENT_ID) IN (${placeholdersForAgentId})`;
      fetchDailyTotalInteractionsQuery += ` AND TRIM(AGENT_ID) IN (${placeholdersForAgentId})`;
      fetchDataQuery += ` AND TRIM(AGENT_ID) IN (${placeholdersForAgentId})`;
      fetchDailyDataQuery += ` AND TRIM(AGENT_ID) IN (${placeholdersForAgentId})`;
    }
    ...
    if (partyId.length > 0) {
      fetchTotalInteractionsQuery += ` AND TRIM(PARTY_ID) IN (${placeholdersForPartyId})`;
      fetchDailyTotalInteractionsQuery += ` AND TRIM(PARTY_ID) IN (${placeholdersForPartyId})`;
      fetchDataQuery += ` AND TRIM(PARTY_ID) IN (${placeholdersForPartyId})`;
      fetchDailyDataQuery += ` AND TRIM(PARTY_ID) IN (${placeholdersForPartyId})`;
    }
    ...
    const binds = {
      fromDate,
      toDate,
      callFrom: selectedCallDuration.from,
      callTo: selectedCallDuration.to,
    ...
    queue.forEach((queueVal, index) => {
      binds[`queue${index + 1}`] = queueVal;
    });
    clientId.forEach((clientVal, index) => {
      binds[`clientId${index + 1}`] = clientVal;
    });
    workTeams.forEach((workTeamsVal, index) => {
      binds[`workTeams${index + 1}`] = workTeamsVal;
    });
    agentId.forEach((agentVal, index) => {
      binds[`agentId${index + 1}`] = agentVal;
    });
    ANI.forEach((ANIVal, index) => {
      binds[`ANI${index + 1}`] = ANIVal;
    });
    partyId.forEach((partyVal, index) => {
      binds[`partyId${index + 1}`] = partyVal;
    });
    // console.log(binds);
...
app.post("/groupedData", async (req, res) => {
  ...
  try {
    ...
    if (workTeams.length > 0) {
      fetchGroupDataCallQuery += ` AND TRIM(WORK_TEAMS) IN (${placeholdersForWorkTeams})`;
      fetchGroupDataACDQuery += ` AND TRIM(WORK_TEAMS) IN (${placeholdersForWorkTeams})`;
      fetchGroupDataCustomerQuery += ` AND TRIM(WORK_TEAMS) IN (${placeholdersForWorkTeams})`;
      fetchGroupDataAgentQuery += ` AND TRIM(WORK_TEAMS) IN (${placeholdersForWorkTeams})`;
      fetchGroupDataSilenceQuery += ` AND TRIM(WORK_TEAMS) IN (${placeholdersForWorkTeams})`;
      fetchGroupDataIVRQuery += ` AND TRIM(WORK_TEAMS) IN (${placeholdersForWorkTeams})`;
      fetchGroupDataOthersQuery += ` AND TRIM(WORK_TEAMS) IN (${placeholdersForWorkTeams})`;
      fetchGroupDataOvertalkQuery += ` AND TRIM(WORK_TEAMS) IN (${placeholdersForWorkTeams})`;
    }

    if (agentId.length > 0) {
      fetchGroupDataCallQuery += ` AND TRIM(AGENT_ID) IN (${placeholdersForAgentId})`;
      fetchGroupDataACDQuery += ` AND TRIM(AGENT_ID) IN (${placeholdersForAgentId})`;
      fetchGroupDataCustomerQuery += ` AND TRIM(AGENT_ID) IN (${placeholdersForAgentId})`;
      fetchGroupDataAgentQuery += ` AND TRIM(AGENT_ID) IN (${placeholdersForAgentId})`;
      fetchGroupDataSilenceQuery += ` AND TRIM(AGENT_ID) IN (${placeholdersForAgentId})`;
      fetchGroupDataIVRQuery += ` AND TRIM(AGENT_ID) IN (${placeholdersForAgentId})`;
      fetchGroupDataOthersQuery += ` AND TRIM(AGENT_ID) IN (${placeholdersForAgentId})`;
      fetchGroupDataOvertalkQuery += ` AND TRIM(AGENT_ID) IN (${placeholdersForAgentId})`;
    }
    ...
    if (partyId.length > 0) {
      fetchGroupDataCallQuery += ` AND TRIM(PARTY_ID) IN (${placeholdersForPartyId})`;
      fetchGroupDataACDQuery += ` AND TRIM(PARTY_ID) IN (${placeholdersForPartyId})`;
      fetchGroupDataCustomerQuery += ` AND TRIM(PARTY_ID) IN (${placeholdersForPartyId})`;
      fetchGroupDataAgentQuery += ` AND TRIM(PARTY_ID) IN (${placeholdersForPartyId})`;
      fetchGroupDataSilenceQuery += ` AND TRIM(PARTY_ID) IN (${placeholdersForPartyId})`;
      fetchGroupDataIVRQuery += ` AND TRIM(PARTY_ID) IN (${placeholdersForPartyId})`;
      fetchGroupDataOthersQuery += ` AND TRIM(PARTY_ID) IN (${placeholdersForPartyId})`;
      fetchGroupDataOvertalkQuery += ` AND TRIM(PARTY_ID) IN (${placeholdersForPartyId})`;
    }
...
app.post("/stackedData", async (req, res) => {
  ...
  try {
    ...
    WHERE 
      TRUNC(startdate) BETWEEN TO_DATE(:fromDate, 'YYYY-MM-DD') AND TO_DATE(:toDate, 'YYYY-MM-DD')
      AND (CALLDURATION >= (:callFrom) AND CALLDURATION <= (:callTo))`;
...
app.post("/getLOB", (req, res) => {
  ...
  const placeholdersForMarketSector = marketSector
    .map((_, i) => `:marketSector${i + 1}`)
    .join(", ");
  const placeholdersForQueue = queue.map((_, i) => `:queue${i + 1}`).join(", ");
  const placeholdersForClientId = clientId
    .map((_, i) => `:clientId${i + 1}`)
    .join(", ");
  const placeholdersForAgentId = agentId
...
AND CAST(startdate AS TIMESTAMP WITH TIME ZONE) <= TO_TIMESTAMP_TZ('${endDate}', 'YYYY-MM-DD HH24:MI:SS TZH:TZM')`;
  ...
  if (queue.length > 0) {
    query += ` AND TRIM(QUEUE) IN (${placeholdersForQueue})`;
  }
  ...
  if (agentId.length > 0) {
    query += ` AND TRIM(AGENT_ID) IN (${placeholdersForAgentId})`;
  }
  ...
  if (partyId.length > 0) {
    query += ` AND TRIM(ANI) IN (${placeholdersForPartyId})`;
  }
   query+=` ORDER BY LOB`
  // Prepare binds for query
  const binds = {};
...
app.post("/getWorkTeams", (req, res) => {
  ...
  if (partyId.length > 0) {
    query += ` AND TRIM(PARTY_ID) IN (${placeholdersForPartyId})`;
  }
...
app.post("/getANI", (req, res) => {
  ...
  if (workTeams.length > 0) {
    query += ` AND TRIM(WORK_TEAMS) IN (${placeholdersForWorkTeams})`;
  }
  if (agentId.length > 0) {
    query += ` AND TRIM(AGENT_ID) IN (${placeholdersForAgentId})`;
  }
...
//   let query = `SELECT DISTINCT TRIM(PARTY_ID) as PARTY_ID FROM CLOUD_STA_IXNS WHERE  
//   CAST(startdate AS TIMESTAMP WITH TIME ZONE) >= TO_TIMESTAMP_TZ('${startDate}', 'YYYY-MM-DD HH24:MI:SS TZH:TZM') 
//   AND CAST(startdate AS TIMESTAMP WITH TIME ZONE) <= TO_TIMESTAMP_TZ('${endDate}', 'YYYY-MM-DD HH24:MI:SS TZH:TZM')`;
